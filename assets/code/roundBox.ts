...
export class RoundBox extends cc.UIRenderer {
  ...
  private fillBuffer() {
    ...
    if (this.node.hasChangedFlags || renderData.vertDirty) {
      ...
      let m00 = m.m00,
        ...
        m13 = m.m13;
      for (
        let i = 0, id = 0, len = vb.length, step = renderData.floatStride;
        i < len;
        i += step, ++id
      ) {
        let x = data[id].x,
  ...
  private updateSpriteFrame() {
    ...
    if (EDITOR) {
      ...
      cc.assetManager.loadAny(
        frame["_atlasUuid"],
        (err: Error, asset: cc.SpriteAtlas) => {
          if (err) {
            this.atlas = null;
            return;
          }
  ...
  private updateSizeMode() {
    ...
    switch (this._sizeMode) {
      case SizeMode["TRIMMED: 原始尺寸裁剪透明像素"]:
        ut.setContentSize(this._spriteFrame["_rect"].size);
        break;
      case SizeMode["RAW: 图片原始尺寸"]:
        ut.setContentSize(this._spriteFrame["_originalSize"]);
        break;
    }
  }
  ...
  private updateVData() {
    ...
    let corner = this._corner;
    data[0].x = lo;
    data[0].y = corner.leftBottom ? bo : b;
    data[1].x = l;
    data[1].y = data[0].y;
    data[2].x = lo;
    data[2].y = b;
    data[3].x = ro;
    data[3].y = corner.rightBottom ? bo : b;
    data[4].x = ro;
    data[4].y = b;
    data[5].x = r;
    data[5].y = data[3].y;
    data[6].x = ro;
    data[6].y = corner.rightTop ? to : t;
    data[7].x = r;
    data[7].y = data[6].y;
    data[8].x = ro;
    data[8].y = t;
    data[9].x = lo;
    data[9].y = corner.leftTop ? to : t;
    data[10].x = lo;
    data[10].y = t;
    data[11].x = l;
    data[11].y = data[9].y;
  ...
  private fitUV(vb: Float32Array) {
    ...
    if (this.spriteFrame["_rotated"]) {
      var uvL = uv[0],
        ...
        uvH = uv[3] - uvB;
      for (
        let i = 3, len = vb.length, step = this._renderData.floatStride;
        i < len;
        i += step
      ) {
        let tmp = vb[i];
